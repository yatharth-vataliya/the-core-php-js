    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <input type="button" id="submit_button" onclick="saveData();" value="save" style="display: none;"
...
<div class="modal fade" id="userinfoModal" tabindex="-1" aria-labelledby="userinfoModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="userinfoModalLabel">User Detail</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="userinfoform">
                    ...
                    <input type="hidden" name="userinfo_id" id="userinfo_id" value="">
                    <div class="row p-2">
                        <div class="col">
                        ...
                        <div class="col">
                            <label class="form-control-label" for="user_email">City</label>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <input type="button" id="userinfo_submit_button" onclick="saveUserInfo();" value="save"
                       style="display: none;"
                       class="btn btn-primary">
            </div>
        </div>
...
<script>
    ...
    function format(d) {
      ...
      <div class="row p-2">
        <div class="col-md-12">
          <button class="btn btn-outline-primary" onclick="saveModal(${d.id});">Add</button>
        </div>
      </div>
    ...
    $(document).ready(function () {
        ...
        $('#tb1 tbody').on('click', 'tr td.details-control', function () {
            ...
            if (row.child.isShown()) {
                ...
                inner_data_table.destroy();
            } else {
                tr.addClass('details');
                row.child(format(row.data())).show();
                let data = row.data();
                inner_data_table = $(`#table_${data.id}`).DataTable({
                    'processing': true,
                    'serverSide': true,
                    'ajax': {
                        'url': 'userinfoapi.php',
                    ...
                    'columns': [
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row, meta) {
                                return `
                                            <button class="btn btn-warning btn-sm" onclick="editUserInfo(this)"><i class="fas fa-edit"></i></button>
                                            <button class="btn btn-danger btn-sm" onclick="deleteUserInfo(this)"><i class="fas fa-trash"></i></button>
                                        `;
                            }
                        }
                    ]
    ...
    user_info_submit_button = document.getElementById('userinfo_submit_button');
    userid = document.getElementById('userid');
    userinfo_id = document.getElementById('userinfo_id');

    function saveModal(user_id) {
        userid.value = user_id;
        user_info_submit_button.style.display = 'block';
        user_info_submit_button.value = 'save';
    ...
    function saveUserInfo() {
        ...
        type = user_info_submit_button.value;
        ...
        if (type == 'save') {
            ...
            http.open("POST", "userinfoapi.php");
            form_data = new FormData();
            form_data.append("type", 'save');
            form_data.append("user_id", user_id);
            form_data.append("state", state.value);
            form_data.append("city", city.value);
            http.send(form_data);
        }
        ...
        if (type == 'update') {
            ...
            form_data.append("state", state.value);
            form_data.append("city", city.value);
            form_data.append("userinfo_id", userinfo_id.value);
            http.send(form_data);
        }
    ...
    function editUserInfo(row) {
        userinfo = inner_data_table.row(row.parentElement.parentElement).data();
        state.value = userinfo.state;
        city.value = userinfo.city;
        userid.value = '';
        userinfo_id.value = userinfo.id;
        user_info_submit_button.value = 'update';
        user_info_submit_button.style.display = 'block';
        $("#userinfoModal").modal('show');
    }

    function deleteUserInfo(row) {
        userinfo = inner_data_table.row(row.parentElement.parentElement).data();
        conf = confirm("Are you sure you want to delete user details ? it can't be revert");
        if(!conf){
            return;
        }
        var http = new XMLHttpRequest();
        ...
        form_data.append("userinfo_id", userinfo.id);
        http.send(form_data);
    }
    ...
    $("#userinfoModal").on('hide.bs.modal', function () {
        ...
        userid.value = '';
        user_info_submit_button.value = 'save';
        user_info_submit_button.style.display = 'none';
    });
