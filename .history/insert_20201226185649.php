...
$st1 = $pdo->prepare('INSERT INTO sort1 (s1_name,s1_price) VALUE (:s1_name,:s1_price)');

$st1 = $pdo->prepare('INSERT INTO sort2 (s2_name,s2_price) VALUES (:s2_name,:s2_price)');

for ($i = 0; $i < 10; $i++) {
	$st1->bindValue(':s1_name', $faker->name);
	$st1->bindValue(':s1_price', random_int(20, 200));

	$st1->execute();

	$st2->bindValue(':s2_name', $faker->name);
	$st2->bindValue(':s2_price', random_int(20, 200));
